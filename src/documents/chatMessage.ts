...
export type SystemChatMessageTypes = Exclude<
  foundry.documents.BaseChatMessage.TypeNames,
  "base"
>;
  ...
  async reroll(): Promise<void> {
    if (this.type !== "action") {
      ui.notifications!.error(
        "Invalid message type. You may only reroll action rolls.",
      );
      return;
    }
    ...
    if (this.system.rerolled) {
      ui.notifications!.error(
        "Cannot reroll a roll that has already been rerolled.",
      );
      return;
    }
    ...
    const newRoll = new Roll(
      `${this.system.diceQuantity - successfulDiceResults.length}d10`,
    );
    ...
    const reroll = Roll.fromTerms([
      new foundry.dice.terms.Die({
        number: this.system.diceQuantity,
        faces: 10,
        results,
      }),
    ]);
